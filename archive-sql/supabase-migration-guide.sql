-- ⛔⛔ DO NOT RUN — 폐기된 초기 설정본 (2026-07-07 표시) ⛔⛔
-- 하단 RLS 정책부(약 451~568행)에 민감 테이블 전면 개방 USING(true)/WITH CHECK(true) 가 있다.
-- 운영은 이후 RLS 하드닝으로 교체됨. 재실행 시 보안 구멍이 다시 열리므로 실행 금지.
-- 스키마 히스토리 참고용으로만 보존.
-- ══════════════════════════════════════════════════════════
-- 콘텐츄어 통역 서비스 - Supabase 마이그레이션 SQL (폐기됨 / 실행 금지)
...
CREATE TYPE notification_type AS ENUM ('system', 'service', 'payment', 'matching', 'settlement', 'chat', 'contract', 'assignment', 'mutual_match', 'rate_change', 'quote', 'marketing', 'points');
