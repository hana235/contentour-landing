...
-- 이 파일은 다수 민감 테이블(01_회원/42_통역계약/43_정산내역/47_결제기록/48_통역사지원서 등)에
-- USING(true) / WITH CHECK(true) 전면 개방 RLS 정책을 포함한다.
-- 운영(jgeqbdrfpekzuumaklvx)은 이후 RLS 하드닝(v2~v7 + 2026-05-29 스냅샷)으로 교체됨.
-- ▶ 이 파일을 재실행하면 모든 로그인 사용자가 남의 데이터를 읽는 보안 구멍이 다시 열린다.
-- ▶ 스키마 히스토리 참고용으로만 보존. 절대 실행하지 말 것.
-- ══════════════════════════════════════════════════════════════
-- 콘텐츄어 랜딩 - 새 Supabase 프로젝트 DB 설정 (폐기됨 / 실행 금지)
-- ══════════════════════════════════════════════════════════════
