...
const interpData = {
    ...
    ae: { country: '아랍에미리트', flag: 'ae', lang: '아랍어 (العربية)', link: 'interpreters.html?country=ae', interpreters: [] },
            ...
            d.interpreters.map(function (p, idx) {
                return '<div class="ip-card" onclick="event.stopPropagation();openIpModal(\'' + code + '\',' + idx + ')">' +
                    ...
                    '<div class="ip-card__tags">' +
                        ...
                        (p.fieldTag ? '<span class="ip-card__tag field">' + escHtml(p.fieldTag) + '</span>' : '') +
                    '</div>' +
...
function openIpModal(countryCode, idx) {
    ...
    html += '<div class="ip-modal__section"><div class="ip-modal__section-title">전문 분야</div><div class="ip-modal__tags">' + p.tags.map(function (t) { return '<span class="ip-modal__tag">' + escHtml(t) + '</span>'; }).join('') + (p.fieldTag ? '<span class="ip-modal__tag field">' + escHtml(p.fieldTag) + '</span>' : '') + '</div></div>';
...
async function checkLoginAndInquiry(name, lang, field, country, photo, role) {
    ...
    area.innerHTML = '<div style="text-align:center;padding:20px;">' +
        ...
        '<button class="ip-modal__cta-btn" style="background:#f5f5f5;color:#333;" onclick="cancelDirectInquiry(\'' + escJsAttr(name) + '\',\'' + escJsAttr(lang) + '\',\'' + escJsAttr((field || '')) + '\',\'' + escJsAttr(country) + '\',\'' + escJsAttr((photo || '')) + '\',\'' + escJsAttr((role || '')) + '\')">취소</button>' +
        '</div>';
}
...
function showDirectInquiry(name, lang, field, country, photo, role) {
    ...
    // ※ 2026-07-15 수정: pf 는 getUserProfile() 이 반환한 01_회원 행 원본이라
    //   컬럼명이 company_name 이다(supabase-config.js:162 select('*')).
    //   pf.company 는 항상 undefined 라 필수값인 회사명 prefill 이 늘 빈칸이었고,
    //   고객이 이미 가입 때 넣은 회사명을 직접견적 때마다 다시 입력해야 했다.
    //   (customer-dashboard.html:1739 는 자체 객체를 만들며 company_name→company 로
    //    매핑해 두어 그쪽 .company 사용은 정상 — 이 파일만 원본 행을 그대로 쓴다.)
    var prefillCompany = pf.company_name || pf.company || '';

    area.innerHTML = '<div class="diq"><div class="diq__head"><div class="diq__interp">' + ((photo && /^https?:\/\//i.test(photo)) ? '<img src="' + escHtml(photo) + '" class="diq__photo">' : avatarHtml('', name, 48)) + '<div><div class="diq__name">' + escHtml(name) + '</div><div class="diq__role">' + escHtml(role) + ' · ' + escHtml(lang) + '</div></div></div><div class="diq__title">직접 견적 의뢰</div></div><div class="diq__form"><div class="diq__row"><div class="diq__field"><label>회사명 <span class="diq__req">*</span></label><input type="text" id="diq-company" placeholder="예: (주)콘텐츄어" value="' + escHtml(prefillCompany) + '"></div><div class="diq__field"><label>담당자명 <span class="diq__req">*</span></label><input type="text" id="diq-name" placeholder="예: 홍길동" value="' + escHtml(prefillName) + '"></div></div><div class="diq__row"><div class="diq__field"><label>이메일 <span class="diq__req">*</span></label><input type="email" id="diq-email" placeholder="예: gildong@company.com" value="' + escHtml(prefillEmail) + '"></div><div class="diq__field"><label>연락처 <span class="diq__req">*</span></label><input type="tel" id="diq-phone" placeholder="예: 010-0000-0000" value="' + escHtml(prefillPhone) + '"></div></div><div class="diq__row"><div class="diq__field"><label>전시회명 <span class="diq__req">*</span></label><input type="text" id="diq-expo" placeholder="예: MEDICA 2026"></div><div class="diq__field"><label>개최지</label><input type="text" id="diq-location" value="' + escHtml(country) + '" placeholder="예: 독일 / 뒤셀도르프"></div></div><div class="diq__row"><div class="diq__field"><label>전시 기간</label><input type="text" id="diq-period" placeholder="예: 2026.11.17 ~ 11.20"></div><div class="diq__field"><label>통역 형태</label><select id="diq-type"><option value="">선택</option><option value="부스 상주">부스 상주(상담 통역)</option><option value="미팅 동행">미팅 동행</option><option value="현장 운영">현장 운영</option><option value="기타">기타</option></select></div></div><div class="diq__field diq__full"><label>요청 내용 <span class="diq__req">*</span></label><textarea id="diq-message" rows="4" placeholder="통역사에게 전달할 요청 사항을 자유롭게 작성해주세요."></textarea></div><div class="diq__actions"><button class="diq__cancel" onclick="cancelDirectInquiry(\'' + escJsAttr(name) + '\',\'' + escJsAttr(lang) + '\',\'' + escJsAttr((field || '')) + '\',\'' + escJsAttr(country) + '\',\'' + escJsAttr((photo || '')) + '\',\'' + escJsAttr((role || '')) + '\')">취소</button><button class="diq__submit" onclick="submitDirectInquiry(\'' + escJsAttr(name) + '\',\'' + escJsAttr(lang) + '\',\'' + escJsAttr((field || '')) + '\')">견적 의뢰 보내기</button></div></div></div>';
    area.scrollIntoView({ behavior: 'smooth', block: 'center' });

    // 전시회명 자동완성 연결
    var diqExpo = document.getElementById('diq-expo');
    if (diqExpo && window.attachExpoAutocomplete) {
        diqExpo.setAttribute('autocomplete', 'off');
        diqExpo.setAttribute('placeholder', '전시회명 또는 국가를 입력하세요');
        window.attachExpoAutocomplete(diqExpo, { location: 'diq-location', period: 'diq-period' });
    }
}
...
function cancelDirectInquiry(name, lang, field, country, photo, role) {
    var area = document.getElementById('ipCtaArea');
    if (!area) return;
    area.innerHTML = '<button class="ip-modal__cta-btn" onclick="showDirectInquiry(\'' + escJsAttr(name) + '\',\'' + escJsAttr(lang) + '\',\'' + escJsAttr((field || '')) + '\',\'' + escJsAttr(country) + '\',\'' + escJsAttr((photo || '')) + '\',\'' + escJsAttr((role || '')) + '\')">이 통역사에게 직접 견적 의뢰</button>';
}
...
async function submitDirectInquiry(interpName, lang, field) {
    ...
    try {
        ...
        var area = document.getElementById('ipCtaArea');
        area.innerHTML = '<div class="diq__success"><div class="diq__success-icon">✅</div><div class="diq__success-title">견적 의뢰가 전송되었습니다!</div><div class="diq__success-info"><strong>' + escHtml(interpName) + '</strong> 통역사에게 직접 견적 의뢰가 전달되었습니다.<br>' + (result.notified ? '통역사에게 알림이 발송되었습니다.' : '확인 후 빠른 시일 내에 회신드리겠습니다.') + '</div><div class="diq__success-detail"><div>문의번호: <strong>' + escHtml(result.inquiryId || '-') + '</strong></div><div>전시회: ' + escHtml(expo) + '</div><div>접수시간: ' + escHtml(createdAtKR) + '</div></div><button class="ip-modal__cta-btn" onclick="closeIpModal()" style="margin-top:16px;">확인</button></div>';
